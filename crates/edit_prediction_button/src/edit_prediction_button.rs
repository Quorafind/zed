    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match all_language_settings.edit_predictions.provider {
            ...
            EditPredictionProvider::Ctab => {
                ...
                div().child(
                    PopoverMenu::new("ctab")
                        .menu(move |window, cx| {
                            this.update(cx, |this, cx| this.build_ctab_context_menu(window, cx))
                                .ok()
                        })
                        .anchor(Corner::BottomRight)
                        .trigger_with_tooltip(
                            IconButton::new("ctab-icon", IconName::Sparkle)
                                .shape(IconButtonShape::Square)
                            ...
                            move |_window, cx| {
                                if !has_auth_token {
                                    ...
                                    Tooltip::for_action("Ctab", &ToggleMenu, cx)
                                }
                            },
                        )
                        .with_handle(self.popover_menu_handle.clone()),
...
impl EditPredictionButton {
    ...
    fn get_available_providers(&self, cx: &App) -> Vec<EditPredictionProvider> {
        ...
        // Ctab is always available (requires auth token in settings)
            ...
            for provider in other_providers {
                ...
                menu = match provider {
                    ...
                    EditPredictionProvider::Ctab => {
                        ...
                        let entry = ContextMenuEntry::new("Ctab")
                            .when(requires_auth_token && !has_auth_token, |this| {
                                this.documentation_aside(
                                    DocumentationSide::Left,
                                    DocumentationEdge::Bottom,
                                    |_| {
                                        Label::new("Configure auth_token in settings to use Ctab")
                                            .into_any_element()
                                    },
                                )
                            })
    ...
    fn build_ctab_context_menu(
        ...
        ContextMenu::build(window, cx, |menu, window, cx| {
            ...
            menu.separator()
                .entry("Configure Ctab Settings", None, move |window, cx| {
                    window.dispatch_action(zed_actions::agent::OpenSettings.boxed_clone(), cx);
                })
        })
    }
